...
function DonationPage() {
  ...
  useEffect(() => {
    const fetchClientKey = async () => {
      try {
        const res = await axios.get(`${url}/api/payment/config`);
  ...
  useEffect(() => {
    const fetchCampaign = async () => {
      try {
        const res = await axios.get(`${url}/api/campaigns/title/${id}`);
  ...
  const handleDonate = async (e) => {
    ...
    if (amount !== "focusable" && amount < 1000) {
      Swal.fire({
        title: "Warning!",
    ...
    setLoading(true);
    try {
      const resp = await axios.post(`${url}/api/payment/create`, {
  ...
  return (
    <>
      ...
      <div className="mt-25 lg:mt-10 flex flex-col items-center gap-5 text-xs lg:text-lg">
        <BackButton className="absolute top-20 left-5" />
        <h2 className="font-bold text-lg">{campaign}</h2>
        <form
          onSubmit={handleDonate}
          className="w-full p-5 rounded shadow-lg flex flex-col "
        >
          <div className="mb-10">
            <p className="font-semibold mb-2">Nominal Donasi</p>
            ...
            <div className={`flex flex-col mt-5 ${showInput ? "" : "hidden"}`}>
              ...
              <input
                ...
                min={1000}
                value={amount}
                className="rounded p-1 border border-blue-400"
          ...
          <div className="mb-10 flex flex-col gap-2">
            ...
            <textarea
              name="message"
              id="message"
              className="p-2 border border-blue-400 rounded h-50"
